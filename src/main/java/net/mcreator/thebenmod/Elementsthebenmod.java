...
public class Elementsthebenmod implements IFuelHandler, IWorldGenerator {
	...
	public void preInit(FMLPreInitializationEvent event) {
		try {
			for (ASMDataTable.ASMData asmData : event.getAsmData().getAll(ModElement.Tag.class.getName())) {
				...
				if (clazz.getSuperclass() == Elementsthebenmod.ModElement.class)
					elements.add((Elementsthebenmod.ModElement) clazz.getConstructor(this.getClass()).newInstance(this));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
		Collections.sort(elements);
		elements.forEach(Elementsthebenmod.ModElement::initElements);
		this.addNetworkMessage(thebenmodVariables.WorldSavedDataSyncMessageHandler.class, thebenmodVariables.WorldSavedDataSyncMessage.class,
	...
	public void onPlayerLoggedIn(net.minecraftforge.fml.common.gameevent.PlayerEvent.PlayerLoggedInEvent event) {
		if (!event.player.world.isRemote) {
			WorldSavedData mapdata = thebenmodVariables.MapVariables.get(event.player.world);
			WorldSavedData worlddata = thebenmodVariables.WorldVariables.get(event.player.world);
			if (mapdata != null)
				thebenmod.PACKET_HANDLER.sendTo(new thebenmodVariables.WorldSavedDataSyncMessage(0, mapdata), (EntityPlayerMP) event.player);
			if (worlddata != null)
				thebenmod.PACKET_HANDLER.sendTo(new thebenmodVariables.WorldSavedDataSyncMessage(1, worlddata), (EntityPlayerMP) event.player);
		}
	}
	...
	public <T extends IMessage, V extends IMessage> void addNetworkMessage(Class<? extends IMessageHandler<T, V>> handler, Class<T> messageClass,
			Side... sides) {
		for (Side side : sides)
			thebenmod.PACKET_HANDLER.registerMessage(handler, messageClass, messageID, side);
		messageID++;
	}
	...
	public static class ModElement implements Comparable<ModElement> {
		...
		protected final Elementsthebenmod elements;
		protected final int sortid;

		public ModElement(Elementsthebenmod elements, int sortid) {
